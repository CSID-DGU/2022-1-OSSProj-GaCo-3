...
class Abyss(Monster):
    def __init__(self, pos, MONSTER_SIZE, groups, obstacle_sprites):
        ...
        self.attack_duration_time = 3000 # 3초에 한 번씩 공격하기
    ...
    def animate(self, df):
        ...
        if 'hurt' in self.status or 'attack' in self.status:
    ...
    def update(self, df):
        ...
        debug("monster hitbox : "+str(self.hitbox), 800, 170)
        debug("monster attackbox : "+str(self.attackBox), 800, 200)
        ...
        if 'attack' in self.status and not self.isDead:
